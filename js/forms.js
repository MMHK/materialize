(function ($) {
  ...
  $.fn.material_select = function (callback) {
    $(this).each(function () {
      ...
      // mobile wrapper
      var mwrapper = $('<div class="select-mobile-wrapper"></div>');

      wrapper.addClass($select.attr('class'));
      mwrapper.addClass($select.attr('class'));
      ...
      options.find('li:not(.optgroup)').each(function (i) {
        /**
         * add mousedown event listener to fix single option iusse
         */
        $(this).on("click mousedown", function (e) {
          ...
          if (!$(this).hasClass('disabled') && !$(this).hasClass('optgroup')) {
            ...
            /**
             * ignore multiple option ousedown event listener
             */
            if (e.type == "mousedown" && multiple) {
              e.stopPropagation();
              return;
            }
      ...
      // if its Mobile
      // Mobile Wrap Elements

      // if(window.devicePixelRatio && window.devicePixelRatio>1){
      if ($(document).width() < 768 && !multiple) {

        $select.wrap(mwrapper);
